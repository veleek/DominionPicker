...
namespace Ben.Dominion.ViewModels
{
       ...
       : NotifyPropertyChangedBase
    {
        ...
        private List<CardGrouping> groupedCards = null;
        ...
        private void Cards_CollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.Action == NotifyCollectionChangedAction.Replace)
            {
                ...
                CardGrouping replacementGroup = this.GroupedCards.First(g => g.Key == replacementCard.Group);
        ...
        [XmlIgnore]
        public List<CardGrouping> GroupedCards
        {
            get
            {
                if (this.groupedCards == null)
                {
                    this.groupedCards = this.cards.OrderBy(c => c.DisplayName).GroupBy(c => c.Group, (g, cards) => new CardGrouping(g, new CardList(cards))).OrderBy(g => g.Key.Type).ToList();
                }
